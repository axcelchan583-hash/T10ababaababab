...
capture log close
log using "`logs'/run_idc_scope_sample_robustness_stata_20260525.log", text replace
...
postfile `handle' ///
    ...
    long N N_city ///
    double mean_y ///
    using "`tables'/idc_scope_sample_robustness_20260525.dta", replace
...
foreach s of local samples {
    ...
    foreach y of local outcomes {
        quietly areg `y' `x' i.province_id#i.year if `cond', absorb(city_id) vce(cluster city_id)
        local b = _b[`x']
        local se = _se[`x']
        local t = `b' / `se'
        ...
        local nc = r(N)
        post `handle' ("`s'") ("province_year_fe") ("`y'") (`b') (`se') (`t') (`p') (e(N)) (`nc') (`mean_y')
        drop `es' `tag'
        ...
        local nc = r(N)
        post `handle' ("`s'") ("provyr_baseyr_fe") ("`y'") (`b') (`se') (`t') (`p') (e(N)) (`nc') (`mean_y')
        drop `es2' `tag2'
    }
}
...
use "`tables'/idc_scope_sample_robustness_20260525.dta", clear
gen sig = cond(p < 0.01, "***", cond(p < 0.05, "**", cond(p < 0.1, "*", "")))
order sample spec outcome coef se t p sig N N_city mean_y
export delimited using "`tables'/idc_scope_sample_robustness_20260525.csv", replace
