...
capture log close
log using "`logs'/run_firm_entry_clean_idc_scope_stata_20260525.log", text replace
...
local firmvars ///
    firm_total_patents firm_active_applicants new_firm_applicants ///
    new_firm_patents firm_incumbent_patents firm_incumbent_applicants ///
    firm_new_share_firm firm_new_share_total firm_patent_share_total ///
    firm_incumbent_share_total new_firm_share_all_new_patents ///
    ln1p_firm_total_patents ln1p_firm_active_applicants ///
    ln1p_new_firm_applicants ln1p_new_firm_patents ///
    ln1p_firm_incumbent_patents ln1p_firm_incumbent_applicants
...
postfile `handle' ///
    ...
    long N N_city ///
    double mean_y ///
    using "`tables'/firm_entry_clean_idc_scope_results_20260525.dta", replace
...
local outcomes ///
    ...
    ln1p_firm_active_applicants ///
    firm_patent_share_total ///
...
foreach sample in all min20 min50 {
    ...
    foreach y of local outcomes {
        ...
        local nc = r(N)
        post `handle' ("`sample'") ("city_year_fe") ("`y'") ///
        ...
        local nc = r(N)
        post `handle' ("`sample'") ("city_trend") ("`y'") ///
            (`b') (`se') (`t') (`p') (e(N)) (`nc') (`mean_y')
        drop `es4' `tag4'
    }
    restore
...
use "`tables'/firm_entry_clean_idc_scope_results_20260525.dta", clear
gen sig = cond(p < 0.01, "***", cond(p < 0.05, "**", cond(p < 0.1, "*", "")))
order sample spec outcome coef se t p sig N N_city mean_y
export delimited using "`tables'/firm_entry_clean_idc_scope_results_20260525.csv", replace
